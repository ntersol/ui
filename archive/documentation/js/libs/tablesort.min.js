    ...
    (a.prototype = {
          ...
          for (e = 0; e < c.cells.length; e++)
            (f = c.cells[e]),
              ...
              'none' !== f.getAttribute('data-sort-method') &&
                ((f.tabindex = 0), f.addEventListener('click', h, !1), null !== f.getAttribute('data-sort-default') && (d = f));
      ...
      sortTable: function(a, g) {
        ...
        if (
          ...
          g ||
            ((o = 'ascending' === o ? 'descending' : 'descending' === o ? 'ascending' : h.options.descending ? 'descending' : 'ascending'),
            ...
            if (!(h.table.tBodies[m].rows.length < 2)) {
              for (p = 0; p < h.table.tBodies[m].rows.length; p++)
                (k = h.table.tBodies[m].rows[p]),
                  'none' === k.getAttribute('data-sort-method') ? (r[s] = k) : q.push({ tr: k, td: d(k.cells[h.col]), index: s }),
                  s++;
              for ('descending' === o ? q.sort(f(j, !0)) : (q.sort(f(j, !1)), q.reverse()), p = 0; p < s; p++)
