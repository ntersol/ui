...
export const cloneAbstractControl = (control: AbstractControl, reset = true) => {
  ...
  if (control instanceof FormGroup) {
    ...
    controlNew = new FormControl(control.value, control.validator, control.asyncValidator);
  } else {
    console.error('Unknown type when cloning abstract control, defaulting to form control');
    controlNew = new FormControl(control.value, control.validator, control.asyncValidator);
  }
