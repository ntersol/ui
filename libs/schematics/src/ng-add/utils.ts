...
export function insertImport(
  ...
  const useStrict = findNodes(rootNode, ts.SyntaxKind.StringLiteral).filter((n: any) => n.text === 'use strict');
  let fallbackPos = 0;
...
export function isImported(source: ts.SourceFile, classifiedName: string, importPath: string): boolean {
  ...
  const matchingNodes = allNodes
    ...
    .filter((imp: any) => imp.moduleSpecifier.kind === ts.SyntaxKind.StringLiteral)
    .filter((imp: any) => {
...
export function getModuleFile(host: Tree, modulePath: any): ts.SourceFile {
