...
const isFormArray = (c: unknown): c is FormArray =>
  !!(c as FormArray)?.controls && Array.isArray((c as FormArray).controls);

/** Is the input a form group  */
const isFormGroup = (f: unknown): f is FormGroup =>
  !!(f as FormGroup)?.controls && !Array.isArray((f as FormArray).controls);
 ...
 */
export const getChangedProps = <t = any>(
  fg: AbstractControl | FormGroup | FormArray | null,
  model?: FormGroup2ApiModel,
): Partial<t> | false => {
 ...
 * @returns
 */
export const modelToHttpRequest = (
  http: HttpClient,
  model: FormGroup2ApiModel,
  control: AbstractControl | FormControl | FormGroup,
) => {
  ...
  if (data[model?.deleteProp || '$$deleted']) {
    ...
    if (model?.apiUrls?.delete) {
      apiUrl =
        typeof model.apiUrls.delete === 'function' ? model?.apiUrls?.delete(parentData, control) : model.apiUrls.delete;
    }
    ...
    if (model?.apiUrls?.patch) {
      apiUrl =
        typeof model.apiUrls.patch === 'function' ? model?.apiUrls?.patch(parentData, control) : model.apiUrls.patch;
    }
    // By default append the unique ID, skip if disabled
    ...
    if (model?.apiUrls?.post) {
      apiUrl =
        typeof model.apiUrls.post === 'function' ? model?.apiUrls?.post(parentData, control) : model.apiUrls.post;
    }
    // By default append the unique ID, skip if disabled
