...
import { getFormControlPath } from './get-form-control.util';
 ...
 * @returns
 */
export const getOrCreateFormControl2 = (form: FormGroup, pathSrc: string): AbstractControl => {
  const path = getFormControlPath(pathSrc);
  const control: AbstractControl | null = form.get(path);
  if (control) {
    return control;
  }
  return createDynamicModel(form, path);
};
 ...
 * @param path
 */
const createDynamicModel = (form: FormGroup, path: string): AbstractControl => {
  const pathArr = path.split('.');
  const current = pathArr[0];
  const controlNext: AbstractControl | null = form.get(current);
  if (!controlNext) {
    form.addControl('current', new FormGroup({}));
  }
  return createDynamicModel(form, path);
};
