...
export namespace NtsState2 {

    export interface BaseStore {
        events$: Observable<Action<unknown, unknown>>;
        dispatch: (a: Action | ApiAction) => void
    }

    export interface ApiBaseStore<t = any> {
        state$: Observable<ApiState<t, any>>
        get: <t>(optionsOverride?: NtsState2.Options) => Observable<t>;
        stateChange: any;
    }
    ...
    export interface ActionCreator<t = any> {
         ...
         * }
         */
        match: (action: Action) => action is Action<t, unknown>;
